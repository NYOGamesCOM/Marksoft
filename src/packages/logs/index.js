...
module.exports = async function(channel, message, options) {
  const init = new Promise(async (resolve) => {
    ...
    // Assign Webhook
    let hook = webhooks.find((w) => w.name === "Marksoft");
    if (!hook) {
      try {
        hook = await channel.createWebhook("Marksoft", {
          avatar: `https://www.youtube.com/channel/UCAqGq94UhaTP34ov1LTsQFw`,
        });
      } catch (e) {
        hook = await channel.createWebhook(
          "Marksoft",
          `https://www.youtube.com/channel/UCAqGq94UhaTP34ov1LTsQFw`
        );
      }
