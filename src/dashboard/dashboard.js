...
module.exports = async (client) => {
    ...
    async function(req, res) {
      ...
      try {
        const login = new MessageEmbed()
          ...
          .setTitle(`Login Logs`)
          .setDescription(
            `\nUser: ${req.user.username}#${req.user.discriminator} \`(${req.user.id
  ...
  app.get("/hangman", (req, res) => {
    renderTemplate(res, req, "hangman.ejs");
  });
  ...
  app.get("/quiz", (req, res) => {
    renderTemplate(res, req, "quiz.ejs");
  });
...
app.post("/redeem/:guildID", checkAuth, async (req, res) => {
  ...
  app.get("/dashboard/:guildID", checkAuth, async (req, res) => {
    ...
    renderTemplate(res, req, "./new/mainpage.ejs", {
      guild: guild,
      alert: `Dashboard was made by https://Marksoft.ro`,
 ...
 */
    ...
    if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
      ...
      if (ticketSettings.ticketToggle == "true") {
        if (
          ...
          if (storedSettings.isPremium == "false") {
            ticketSettings.ticketFooter = "Powered by Marksoft.ro";
          } else {
          ...
          let reactionFooter = "Powered by Marksoft.ro";

          let footer = "Powered by Marksoft.ro";
          // eslint-disable-next-line no-unused-vars
          ...
          if (storedSettings.isPremium == "false") {
            ticketEmbed.setFooter({ text: `Powered by Marksoft.ro` });
          } else {
            if (checkFooter) {
  ...
  app.post("/dblwebhook", webhook.middleware(), async (req) => {
    ...
    const msg = new Discord.MessageEmbed()
      ...
      .setDescription(
        `Thank you **${apiUser.username}#${apiUser.discriminator}** (${apiUser.id}) for voting **Marksoft**!`
      );
    Hook.sendCustom(msg);
      ...
      voteUser.send({
        embeds: [
          new Discord.MessageEmbed()
            ...
            .setDescription(
              `Thank you **${apiUser.username}#${apiUser.discriminator}** (${apiUser.id
              }) for voting **Marksoft**! \n\nVote #${voteNumber + 1}`
            ),
        ],
      });
