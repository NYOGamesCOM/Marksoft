// Want to use or contribute to this? https://github.com/Glitchii/MessageEmbed
...
window.onload = () => {
  document
    .querySelectorAll("img.clickable")
    .forEach((e) =>
      e.addEventListener("click", (el) => window.open(el.target.src))
    );
  let editorHolder = document.querySelector(".editorHolder"),
    ...
    json = {
      content:
        "This is a Message Embed <:success:788438144818217000>\nEverything is _Customizable_ including **Emojis**\nYou can~~not~~ also do `this`.```py\nAnd ('this')```",
      embed: {
        title: "Hello! :wave:",
        description:
          "You can use [links](https://Marksoft.ro) or emojis :smile: :eyes:\n```\nAnd also code blocks\n```",
        color: 4321431,
  ...
  window.editor = CodeMirror(
    (elt) => editorHolder.parentNode.replaceChild(elt, editorHolder),
    {
  ...
  let notif = document.querySelector(".notification"),
    ...
    makeShort = (txt, length, mediaWidth) => {
      if (
        mediaWidth &&
        window.matchMedia(`(max-width:${mediaWidth}px)`).matches
      )
        return txt.length > length - 3
          ? txt.substring(0, length - 3) + "..."
          : txt;
      return txt;
    },
    error = (msg, time) => {
    ...
    allGood = (e) => {
      let re = /"((icon_)?url")(: *)("(?!https?:\/\/).+?")/g.exec(
        editor.getValue()
      );
      if (re)
        return error(
          `URL should start with <code>https://</code> or <code>http://</code> on this line <span class="inline full">${makeShort(
            re[0],
            30,
            600
          )}</span>`
    ...
    markup = (txt, opts) => {
      ...
      if (opts.replaceEmojis)
        txt = txt.replace(
          /(?<!code(?: \w+=".+")?>[^>]+)(?<!\/[^\s"]+?):((?!\/)\w+):/g,
          (match, x) => (x && emojis[x] ? emojis[x] : match)
        );
      txt = txt
        .replace(
          /(?<=\n|^)\s*&#62;\s+([^\n]+)/g,
          '<div class="blockquote"><div class="blockquoteDivider"></div><blockquote>$1</blockquote></div>'
        )
        .replace(/\n/g, "<br>");
      return txt;
    },
    ...
    imgSrc = (elm, src, remove) =>
      remove
        ? elm.style.removeProperty("content")
        : (elm.style.content = `url(${src})`),
    toObj = (jsonString) =>
      JSON.parse(
        jsonString.replace(
          /\\"|"(?:\\"|[^"])*"|(\/\/.*|\/\*[\s\S]*?\*\/)/g,
  ...
  buildGui = (object, opts) => {
    gui.innerHTML = `
                ...
                <div class="edit">
                    <textarea class="editContent" placeholder="Message content" autocomplete="off">${encodeHTML(
      object.content || ""
    )}</textarea>
                </div>
        ...
        object.embed?.author?.name || ""
                    ...
                    <div class="item title inlineField">
                        ...
                        <input class="editTitle" type="text" placeholder="Title" autocomplete="off" value="${encodeHTML(
        object.embed?.title || ""
      )}">
                    </div>
                    <div class="item description"><p class="ttle">Description</p></div>
                    <div class="edit">
                        <textarea class="editDescription" placeholder="Embed description" autocomplete="off">${encodeHTML(
        ...
        object.embed?.footer?.icon_url || ""
                            ...
                            <div class="editName">
                                <input class="editFooterText" type="text" value="${encodeHTML(
    ...
    let fieldsEditor = gui.querySelector(".fields ~ .edit"),
      addField = `
                    <div class="addField">
                        <p>New Field</p>
                    ...
                    <div class="field">
                        ...
                        <div class="fieldInner">
                            <div class="designerFieldName">
                                <input type="text" placeholder="Field name" autocomplete="off" value="${encodeHTML(
              f.name
            )}">
                            </div>
                            <div class="designerFieldValue">
                                <textarea placeholder="Field value" autocomplete="off">${encodeHTML(
              f.value
            )}</textarea>
                            </div>
                        </div>
      ...
      e.addEventListener("click", (el) => {
        ...
        if (field) {
          let i = Array.from(fields.children).indexOf(field),
            jsonField = object.embed.fields[i];
          if (jsonField) {
    ...
    document.querySelectorAll(".gui > .item").forEach((e) => {
      e.addEventListener("click", (el) => {
        ...
        if (elm.classList.contains("active"))
          window.getSelection().anchorNode !== elm &&
    ...
    document.querySelector(".addField").addEventListener("click", () => {
      ...
      if (arr.length >= 25)
        return error(`You can't have over 25 embed fields.`, 5000);
      arr.push({ name: "Field name", value: "Field value", inline: false });
      json.embed.fields = arr;
      update(json);
      buildGui(json, {
        newField: true,
        activate: document.querySelectorAll(".gui > .item.active"),
      });
    });
    ...
    gui.querySelectorAll("textarea, input").forEach((e) =>
      e.addEventListener("input", (el) => {
        ...
        if (field) {
          let jsonField =
            json.embed.fields[Array.from(fields.children).indexOf(field)];
          if (jsonField) {
          ...
          switch (el.target) {
            ...
            case title:
              ...
              break;
            case authorName:
              (json.embed.author ??= {}), (json.embed.author.name = v);
              break;
            case authorLink:
              (json.embed.author ??= {}),
                (json.embed.author.icon_url = v),
                imgSrc(el.target.previousElementSibling, v);
              break;
            case desc:
              json.embed.description = v;
              break;
            case thumbLink:
              (json.embed.thumbnail ??= {}),
                (json.embed.thumbnail.url = v),
                imgSrc(
                  el.target.closest(".editIcon").querySelector(".imgParent"),
                  v
                );
              break;
            case imgLink:
              (json.embed.image ??= {}),
                (json.embed.image.url = v),
                imgSrc(
                  el.target.closest(".editIcon").querySelector(".imgParent"),
                  v
                );
              break;
            case footerText:
              (json.embed.footer ??= {}), (json.embed.footer.text = v);
              break;
            case footerLink:
              (json.embed.footer ??= {}),
                (json.embed.footer.icon_url = v),
                imgSrc(el.target.previousElementSibling, v);
              break;
          }
        }
    ...
    if (opts?.activate) {
      ...
      Array.from(elements)
        ...
        .forEach((clss) =>
          document
            .querySelectorAll(clss)
            .forEach((e) => e.classList.add("active"))
        );
    } else
      [".item.author", ".item.description"].forEach((clss) =>
        document.querySelector(clss).classList.add("active")
      );
          ...
          .then((res) => {
            ...
            imgSrc(
              e.target.previousElementSibling.querySelector(
                ".editIcon > .imgParent"
              ) || e.target.closest(".editIcon").querySelector(".imgParent"),
              res.link
            );
            let input =
              e.target.previousElementSibling.querySelector(
  ...
  update = (data) => {
    try {
      if (!data.content) embedContent.classList.add("empty");
        ...
        embedContent.innerHTML = markup(encodeHTML(data.content), {
          replaceEmojis: true,
        });
      ...
      if (data.embed && Object.keys(data.embed).length) {
        ...
        if (e.description)
          display(
            embedDescription,
            markup(encodeHTML(e.description), {
              inEmbed: true,
              replaceEmojis: true,
            })
          );
        ...
        if (e.color)
          embed.closest(".embed").style.borderColor = encodeHTML(
        ...
        if (e.thumbnail && e.thumbnail.url)
          (embedThumbnail.src = encodeHTML(e.thumbnail.url)),
            (embedThumbnail.style.display = "block");
        else hide(embedThumbnail);
        if (e.image && e.image.url)
          (embedImage.src = encodeHTML(e.image.url)),
        ...
        if (e.footer && e.footer.text)
          ...
          display(
            embedFooter,
            `<span class="embedFooterText">${encodeHTML(
              tstamp(e.timestamp)
            )}</span></div>`,
            "flex"
          );
        ...
        if (e.fields) {
          ...
          e.fields.forEach((f) => {
            if (f.name && f.value) {
              if (!f.inline) {
                let el = embedFields.insertBefore(
          ...
          if (len === 2 || (len > 3 && len % 2 !== 0)) {
            let children = Array.from(embedFields.children),
              arr = children.filter(
                (x) => x === children[len] || x === children[len - 1]
              );
  ...
  let colRight = document.querySelector(".colRight"),
    removePicker = () => colRight.classList.remove("picking");
  ...
  document
    .querySelectorAll(".markup pre > code")
    .forEach((block) => hljs.highlightBlock(block));
  document.querySelectorAll(".done .selectOptions .channel").forEach((e) =>
    e.addEventListener("click", (el) => {
      ...
      fetch(`/dashboard/${guildID}/embeds`, {
        ...
        body: JSON.stringify({
          ...
          guild: guildID,
          json,
          to: el.target.querySelector("span").getAttribute("data-data"),
        }),
      })
        .then((res) => res.text())
        .then((res) => {
  ...
  document
    .querySelector(".pickerToggle")
    .addEventListener("click", () => colrs.classList.toggle("display"));
  update(json);
