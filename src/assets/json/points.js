...
module.exports = class extends Command {
    ...
    async run(message, args) {
        ...
        leaderboardMessage += `No.  | Username            | Points\n`;
        leaderboardMessage += `-----|---------------------|----------\n`;
        if (guildUsers.length === 0) {
            leaderboardMessage += "No one has had a correct answer yet!";
        } else {
        ...
        if (totalPages > 1) {
            ...
            collector.on('collect', async interaction => {
                ...
                newLeaderboardMessage += `No.  | Username            | Points\n`;
                newLeaderboardMessage += `-----|---------------------|----------\n`;
                //leaderboardMessage += `No.  | Username             | Usage | Total 69s\n`;
                //leaderboardMessage += `-----|----------------------|-------|----------\n`;
                if (guildUsers.length === 0) {
                    newLeaderboardMessage += "NNo one has had a correct answer yet!";
                } else {
