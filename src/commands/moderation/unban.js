  ...
  async run(message, args) {
    ...
    if (id.toString().toLowerCase() == "all") {
      ...
      if (!array || !array.length) {
        ...
        const embed = new MessageEmbed()
          .setDescription(
            `${client.emoji.success} | ${language.unbanSuccess} **${array.length
      ...
      if (!rgx.test(id)) {
        ...
        if (user1) {
          ...
          if (userrz) {
            const embed = new MessageEmbed()
              .setDescription(
                `${client.emoji.success} | ${language.unbanSuccess} ${userrz.tag
      ...
      const embed = new MessageEmbed()
        .setDescription(
          `${client.emoji.success} | ${language.unbanSuccess} ${userr.tag} ${logging && logging.moderation.include_reason === "true"
            ? `\n\n**Reason:** ${reason}`
            : ``
          }`
        )
