...
module.exports = class extends Command {
    ...
    async run(message, args) {
        ...
        // Sort users by the usage in descending order
        if (Array.isArray(guildUsers)) {
            guildUsers.sort((a, b) => b.usage - a.usage);
        } else {
        ...
        leaderboardMessage += `No.  | Username            | Usage\n`;
        leaderboardMessage += `-----|---------------------|----------\n`;
        if (guildUsers.length === 0) {
            ...
            for (let i = startIndex; i < endIndex; i++) {
                ...
                leaderboardMessage += `${(i + 1).toString().padEnd(5, ' ')}| ${user.username.padEnd(20, ' ')}| ${user.usage.toString().padEnd(5, ' ')}\n`;
            }
        }
        ...
        if (totalPages > 1) {
            ...
            collector.on('collect', async interaction => {
                ...
                newLeaderboardMessage += `No.  | Username            | Usage\n`;
                ...
                if (guildUsers.length === 0) {
                    ...
                    for (let i = newStartIndex; i < newEndIndex; i++) {
                        const user = guildUsers[i];
                        newLeaderboardMessage += `${(i + 1).toString().padEnd(5, ' ')}| ${user.username.padEnd(20, ' ')}| ${user.usage.toString().padEnd(5, ' ')}\n`;
